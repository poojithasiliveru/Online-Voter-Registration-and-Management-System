...
router.post("/login", async (req, res) => {
  try {
    ...
    console.log("Received aadharCardNumber:", aadharCardNumber);
...
router.get("/voting-status", jwtAuthMiddleware, async (req, res) => {
  try {
    ...
    const userId = userData.id;
    const user = await User.findById(userId);
    // Check if the user has voted (you need to implement this logic based on your application requirements)
    const hasVoted = user.hasVoted; // Assuming the user model has a field called hasVoted
